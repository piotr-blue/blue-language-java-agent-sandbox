...
- Contract processor lookup by BlueId string now also supports provider-derived type-chain fallback for handler/channel/marker registries (`ContractProcessorRegistryTest#lookupByBlueIdStringSupportsProviderDerivedLookups`), reducing direct-id-only gaps.
...
- Contract-loader parity now includes unsupported-contract MustUnderstand behavior, built-in contract loading coverage, provider-derived handler loading via type-chain lookup, and custom-handler missing-channel fatal validation (`ContractLoaderParityTest`) in addition to composite-cycle/missing-child/nested-acyclic loader tests and MyOS-marker loader tests.
...
- Channel-runner parity now includes inactive-scope handler gating semantics from JS (`allowTerminatedWork`), non-match fast-path no-op behavior, handler-level `matches(...)` gating, and fatal runtime-handler termination behavior (with checkpoint non-persistence for the failing delivery), covered in `ChannelRunnerTest#allowsHandlersToRunWhenScopeInactiveOnlyIfAllowTerminatedWorkIsTrue`, `#doesNothingWhenExternalChannelDoesNotMatch`, `#skipsHandlersWhoseMatchesPredicateReturnsFalse`, and `#entersFatalTerminationWhenHandlerThrowsRuntimeException`.
...
- Workflow matcher parity now also resolves provider-backed subtype chains for explicit `blueId` filter-property matching (including candidate-side `properties.blueId` values), covered by `WorkflowContractSupportTest#matchesEventFilterResolvesProviderBackedBlueIdPropertyChains` and `#matchesEventFilterSupportsCandidatePropertyBlueIdField`.
