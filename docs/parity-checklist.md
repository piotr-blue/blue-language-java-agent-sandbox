...
- Channel-runner parity now includes inactive-scope handler gating semantics from JS (`allowTerminatedWork`) and fatal runtime-handler termination behavior (with checkpoint non-persistence for the failing delivery), covered in `ChannelRunnerTest#allowsHandlersToRunWhenScopeInactiveOnlyIfAllowTerminatedWorkIsTrue` and `#entersFatalTerminationWhenHandlerThrowsRuntimeException`.
