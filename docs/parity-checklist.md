...
- Expression utility parity still needs closer picomatch-equivalent semantics for complex glob patterns (brace expansion with delimiter-aware option splitting across nested parentheses/character classes while skipping class-literal braces/commas, extglob parsing that preserves escaped delimiters and character-class parentheses, character classes including escaped `]` literals, escaped literal brace/wildcard/extglob markers, escaped extglob option separators/closures, and extglob `@(...)`, `?(...)`, `+(...)`, `*(...)`, `!(...)` are supported, including hidden-segment handling for `dot=false` with explicit character-class dot matches and numeric/alphabetic brace-range expansion with optional step increments, but advanced nested/edge picomatch semantics remain partial).
- Registry/type-chain lookup now supports inline derived type-chain fallback and provider-backed repository type-chain fallback during node→class resolution (workflow, operation handler, and operation marker contracts), including fallback to contract-node `blueId` / `properties.blueId` when `type` is absent, and workflow event/type matching now also supports provider-backed semantic subtype fallback for explicit `blueId` filter nodes (including candidate `properties.blueId` fields); broader semantic subtype matching outside these paths remains partial.
...
- Contract processor lookup now also supports node-driven contract-level `blueId` fallbacks (direct and provider-derived) when `type` chains are absent (`ContractProcessorRegistryTest#lookupByNodeBlueIdFieldSupportsDirectAndProviderDerivedLookups`), further reducing class-only lookup gaps.
