...
- Sequential workflow operation request-type matching now supports provider-backed semantic subtype chains (beyond inline `type.type...` nodes), with integration + matcher regression coverage in `SequentialWorkflowOperationProviderTypeChainIntegrationParityTest` and `WorkflowContractSupportTest`.
