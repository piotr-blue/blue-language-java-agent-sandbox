...
- QuickJS expression helper parity now aligns standalone/template detection with JS multiline/nested-brace semantics (`isExpression`/`containsExpression`) and honors `shouldDescend` on the current pointer before evaluating node values (`QuickJsExpressionUtilsTest` regression coverage).
