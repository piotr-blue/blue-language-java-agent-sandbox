...
- QuickJS path-predicate parity now also covers brace-range step expansions (`{1..7..3}`, `{a..g..2}`) and rejects invalid zero-step ranges (`{1..3..0}`), with direct `QuickJsExpressionUtilsTest` coverage.
