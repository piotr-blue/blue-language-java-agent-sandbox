...
- Expression utility parity still needs closer picomatch-equivalent semantics for complex glob patterns (brace expansion, character classes, and extglob `@(...)`, `?(...)`, `+(...)`, `*(...)`, `!(...)` are supported, including hidden-segment handling for `dot=false` with explicit character-class dot matches and basic numeric/alphabetic brace-range expansion, but advanced nested/edge picomatch semantics remain partial).
