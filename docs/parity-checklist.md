...
- Contract processor lookup now also supports node-driven inline `type.type...` BlueId-chain resolution for handler/channel/marker processor matches (`ContractProcessorRegistryTest#lookupByNodeTypeChainSupportsDerivedBlueIds`), reducing class-only lookup gaps.
