...
- Channel-runner parity now includes inactive-scope handler gating semantics from JS (`allowTerminatedWork`), non-match fast-path no-op behavior, and fatal runtime-handler termination behavior (with checkpoint non-persistence for the failing delivery), covered in `ChannelRunnerTest#allowsHandlersToRunWhenScopeInactiveOnlyIfAllowTerminatedWorkIsTrue`, `#doesNothingWhenExternalChannelDoesNotMatch`, and `#entersFatalTerminationWhenHandlerThrowsRuntimeException`.
...
- QuickJS path-predicate parity now also covers brace-range step expansions (`{1..7..3}`, `{7..1..3}`, `{a..g..2}`, `{g..a..2}`), treats negative step declarations as absolute magnitudes (`{1..7..-2}`), and rejects invalid zero-step ranges (`{1..3..0}`), with direct `QuickJsExpressionUtilsTest` coverage.
...
- Sequential workflow operation request parity now also verifies default `allowNewerVersion=true` semantics when the flag is omitted from the operation request payload (`SequentialWorkflowProcessorTest#sequentialWorkflowOperationDefaultsAllowNewerVersionWhenFlagMissing`).
...
- Golden fixture coverage now also includes provider-backed operation-request subtype-chain matching (`provider-derived-operation-request-type-chain.yaml`) to lock parity in the fixture harness path (`ParityFixturesTest`).
- Golden fixture coverage now also includes provider-backed operation-definition subtype-chain matching (`provider-derived-operation-definition-type-chain.yaml`) to lock provider-derived operation-type parity in the fixture harness path (`ParityFixturesTest`).
- Golden fixture coverage now also includes provider-backed workflow event-filter subtype matching (`provider-derived-workflow-event-filter-type-chain.yaml`) to lock provider-chain filtering parity in the fixture harness path (`ParityFixturesTest`).
- Golden fixture coverage now also includes provider-backed operation-workflow event-filter subtype matching (`provider-derived-operation-event-filter-type-chain.yaml`) to lock provider-chain filtering parity for sequential workflow operations in the fixture harness path (`ParityFixturesTest`).
