...
- Expression utility parity still needs closer picomatch-equivalent semantics for complex glob patterns (brace expansion, character classes, and extglob `@(...)`, `?(...)`, `+(...)`, `*(...)`, `!(...)` are supported, including improved hidden-segment handling for `dot=false` with explicit character-class dot matches, but advanced nested/edge picomatch semantics remain partial).
