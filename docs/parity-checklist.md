...
- Registry/type-chain lookup now supports inline derived type-chain fallback and provider-backed repository type-chain fallback during node→class resolution (workflow, operation handler, and operation marker contracts), and workflow event/type matching now also supports provider-backed semantic subtype fallback for explicit `blueId` filter nodes (including candidate `properties.blueId` fields); broader semantic subtype matching outside these paths remains partial.
