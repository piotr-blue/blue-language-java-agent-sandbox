...
public final class PoliciesBuilder {
    ...
    public PoliciesBuilder operationRateLimit(String operationName, int maxCalls, String window) {
        Node rateLimitRoot = policies.get("operationRateLimit");
        if (rateLimitRoot == null) {
            rateLimitRoot = new Node();
            policies.put("operationRateLimit", rateLimitRoot);
        }
        rateLimitRoot.properties(operationName, new Node()
                .properties("maxCalls", new Node().value(maxCalls))
                .properties("window", new Node().value(window)));
        return this;
    }
}
